...
//		return this.words.getOrDefault(word, this.getDefault())[this.HAM]/(double) this.HAM_SIZE;
	...
	public double calcSpam(String[] words)
	{
		...
		for(String word:words)
		{
			a*=this.calcSpam(word);
			b1*=this.calcHam(word);
			b2*=this.calcSpam(word);
		}
	...
	/**
	 * Calculates the {@literal n} most frequent spam words in a mail.
	 * Only works after reading a single mail!
	...
	/**
	 * Calculates the {@literal n} most frequent ham words in a mail.
	 * Only works after reading a single mail!
	 * @param max number of the most frequent words
	 * @return sorted list of the {@literal n} most frequent words
	 */
	...
	private void showTop(int max, int index)
	{
		...
		for(Tuple tuple:top)
		{
			System.out.println(tuple + " ham: " + this.calcHam(tuple.getWord()) + " spam: " + this.calcSpam(tuple.getWord()));
		}
	}
	...
	private String cleanText(String rawText)
	{
		...
		//Punctuation:
		String plainText = rawText.replaceAll("\\p{Punct}+", ""); //Source: http://stackoverflow.com/a/17531480
